...
if (isset($_GET['fetch_rooms'])) {
    ...
    if($checkin_date == $checkout_date){
       echo "<h3 class='text-center text-danger'>Invaild Dates Entered!</h3>";
       exit;
    }
    else if( $checkout_date <  $checkin_date){
       echo "<h3 class='text-center text-danger'>Invaild  Entered!</h3>";
       exit;
    }
...
// facilities data decode 
   $facility_list = json_decode($_GET['facility_list'],true);
    ...
    // Fetching settings to check if the website is shutdown or not
    ...
    if($chk_avail['checkin']!='' && $chk_avail['checkout']!='')
    {
        ...
        if(($room_data['num_room'] - $tb_fetch['total_bookings'])==0){
           continue;
        }
    ...
    // Get facilities of room with filters
        ...
        // If the site is not shut down, allow the booking button
        $book_btn = "";
        if (!$settings_r['shutdown']) {
            $login = isset($_SESSION['login']) && $_SESSION['login'] === true ? 1 : 0;
            $book_btn = "<button onclick='checkLoginToBook($login, $room_data[id])' class='btn btn-sm w-100 text-white custom-bg shadow mb-2'>Book now</button>";
        }
            ...
            <div class='row g-0 p-3 align-items-center'>
                ...
                <div class='col-md-2 text-center'>
                    <div class='card-body'>
                        ...
                        $book_btn
                        <a href='room_details.php?id=$room_data[id]' class='btn btn-sm w-100 btn-outline-dark shadow'>More details</a>
                    </div>
                </div>
            </div>
        </div>";
    ...
    // Output rooms or no rooms message
    if ($count_rooms > 0) {
        echo $output;
    } else {
