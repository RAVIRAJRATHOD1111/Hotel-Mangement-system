        ...
        <div>
          ...
          <div>
            <div class=" d-flex align-items-center mb-2">
              <img src="images/1/star.png" width="30px">
  ...
  <script>
    ...
    function Check_availability() {
      ...
      if (checkin_val != '' && checkout_val != '') {
        ...
        // xhr.onload = function() {
        //   try {
        //     let data = JSON.parse(this.responseText); // Ensure valid JSON is parsed
        //     if (data.status === 'check_in_out_equal') {
        //       pay_info.innerText = "You cannot check-out on the same date!";
        ...
        //       // Only proceed if the response is successful and contains the expected data
        //       pay_info.innerHTML = "No. of Days: " + data.days + "<br>Total Amount to Pay: ₹" + data.payment;
        //       pay_info.classList.replace('text-danger', 'text-dark');
        //       booking_form.elements['pay_now'].removeAttribute('disabled');
        //     } else {
        //       pay_info.innerText = "Unexpected response from the server.";
        //     }
        //   } catch (e) {
        //     console.error("Response is not valid JSON:", this.responseText);
        //     pay_info.innerText = "An error occurred while processing the request.";
        //   }
        ...
        xhr.onload = function() {
            ...
            if (data.status == 'check_in_out_equal') {
                pay_info.innerText = "You canot check-outon the same date!";
            }
            else if (data.status == 'check_out_earlier') {
                pay_info.innerText = " check-out date is earlier then check-in date!";
            } 
            else if (data.status == 'check_in_earlierr') {
                ...
                pay_info.innerHTML = "No. of Days:" + data.days + "<br>Total Amout to Pay: ₹" + data.payment;
                pay_info.classList.replace('text-danger', 'text-dark');
                booking_form.elements['pay_now'].removeAttribute('disabled');
            }
            pay_info.classList.remove('d-none');
