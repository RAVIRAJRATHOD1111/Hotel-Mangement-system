...
if (isset($_POST['add_room'])) {
    ...
    $q1 = "INSERT INTO `rooms` (`name`, `area`, `price`, `num_room`, `adult`, `children`, `description`) VALUES (?,?,?,?,?,?,?)";
    $values = [$frm_data['name'], $frm_data['area'], $frm_data['price'], $frm_data['num_room'], $frm_data['adult'], $frm_data['children'], $frm_data['desc']];
...
if (isset($_POST['edit_room'])) {
    ...
    $frm_data = filteration($_POST);
    $flag = 0;
    $q1 = "UPDATE `rooms` SET `name`=?,`area`=?,`price`=?,`num_room`=?,`adult`=?,`children`=?,`description`=? WHERE `id` = ?";
    $values = [$frm_data['name'], $frm_data['area'], $frm_data['price'], $frm_data['num_room'], $frm_data['adult'], $frm_data['children'], $frm_data['desc'], $frm_data['room_id']];
